...
```
4. Global placement results

The global placement results (both .def and .odb) are generated in:
```bash
scripts/OR_scripts/${DESIGN_NAME}/${METHOD}.odb
```
We also release .odb global placement results generated by MLBuf-RePlAce, Ad-Hoc baseline, RePlAce and TD-RePlAce in the same directory.
