...
1. Replace the default placement ODB file in the following directory with the one generated by MLBuf-RePlAce
```bash
flow/results/{platform}/{design}/base/3_3_place_gp.odb
```


2. After replacing this file, continue the standard ORFS flow (CTS → routing → signoff) to obtain post-route PPA results.

3. All metrics (timing, power) are available in:
```bash
