...
def calculate_updated_features(x_next, device):
    ...
    driver_out_slew = (r_drvr + wire_length_m * wire_res) * driver_output_cap * elmore_k *0.01
...
def build_tree_bottom_up(
    ...
    for i in range(num_levels):
        ...
        bufs_id = all_buffers[i]  # parent buffer id
        if current_level > 0:
    ...
    if autoBuf:
        # auto buffer
        has_buffer_child = any(ch.node_type == "Buffer" for ch in driver_node.children)
        if not has_buffer_child and len(driver_node.children) > 0:
            import random
            jitter_x = random.uniform(-0.7, 0.7)
            jitter_y = random.uniform(-0.7, 0.7)
            if len(driver_node.children) ==2:
                bufID_define = 2
            elif len(driver_node.children) < 5:
                bufID_define = 3
            else:
                bufID_define = 5
            net_name_str = net_name[0]
            auto_buf = TreeNode(node_type="Buffer",
                                x=jitter_x,
                                y=jitter_y,
                                manh_dist=abs(jitter_x) + abs(jitter_y),
                                in_slew=0,
                                out_slew=0,
                                in_cap=0,
                                out_cap=0,
                                max_out_cap=0,
                                fanout=float(len(driver_node.children)),
                                buf_id=bufID_define,  # auto buffer
                                name="AutoBuffer")
                                # pin_name=net_name_str+"BUF_AUTO")
            old_children = driver_node.children
            driver_node.children = [auto_buf]
            auto_buf.parent = driver_node
            for ch in old_children:
                ch.parent = auto_buf
            auto_buf.children = old_children
